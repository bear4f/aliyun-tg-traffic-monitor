...
卡片上的速度是**最近的跑量速度**，取「近 24 小时」与「近 7 天」中**较大**的一个——突然开始跑量必须立刻反映出来，所以宁可偏高。它**不是**本月累计平均值。

因此会出现这种看似矛盾的情况，且是正确的：

| 机器 | 本月已用 | 卡片显示 | 本月累计 ÷ 天数 |
|---|---|---|---|
| A | 92 GB | 近 7 天 1.7 GB/日 | 3.2 GB/日 |
| B | 48 GB | 近 24 小时 4.8 GB/日 | 1.7 GB/日 |

A 月初跑得猛、最近慢下来了；B 整月很闲、昨天开始猛跑。**月底预测关心的是接下来两天会怎样，不是过去 28 天的平均**，所以 B 的预测值反而更高。三个口径都在机器详情页里，可以直接对比。
