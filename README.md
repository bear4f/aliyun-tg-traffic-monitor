...
🛡️ 熔断 95% 开 · 🗓️ 下月开机 关
📈 日均 8.3 GB · 🔮 预计月底 191 GB · ⚠️ 约 07-24 触线

⚫ Aliyun SG-1 · 已关机
███████████░ 91.3% 🔴
182.54 GB / 200.00 GB · 剩余 17.46 GB
🛡️ 熔断 90% 开（本账期已触发🛑） · 🗓️ 下月开机 开
📈 日均 9.6 GB · 🔮 预计月底 219 GB · 🛑 已达熔断线

[🔄 刷新全部]      [📋 事件记录]
[🟢 Aliyun HK-1 · 79.2%] [⚫ Aliyun SG-1 · 91.3%]
[🛠️ 全局设置]      [ℹ️ 帮助]
```

主面板是只读总览：数据新鲜度按延迟着色（🟢/🟡/🔴），月底用量与触线日期按 24 小时 / 7 日滚动均值预测。开关机、重启在点入机器详情页后操作，且都有二次确认；自动熔断采用双阈值——普通线（默认 95%）需连续两次检查确认，紧急线（默认 98%，`emergency_shutdown_percent`）一次即关，防止单次异常数据误关机。`📋 事件记录` 保留最近 50 条熔断、开关机、配置修改与查询异常。所有层级都有 `🏠 返回主面板`。
